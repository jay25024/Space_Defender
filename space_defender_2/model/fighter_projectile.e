note
	description: "Summary description for {FIGHTER_PROJECTILE}."
	author: ""
	date: "$Date$"
	revision: "$Revision$"
...
feature
     ...
     local
         ...
         do
               ...
               across  1 |..| 10 is p loop
                    if position.column-p < 1 then
		             ...
		             across model.friendly_projectile is f loop
			                                         ...
			                                         model.fill_enemy_proj_toggle_string("A enemy projectile(id:"+id.out+") moves: ["+model.array_row[position.row]+","+position.column.out+"] -> ["+model.array_row[position.row]+","+(position.column-p).out+"]%N")
          ...
          position.column:= position.column- 10
          if position.column < 1 and( position.column +10)>=1 and interaction =false and status = true then
          ...
          elseif interaction = false and status = true then
              	model.fill_enemy_proj_toggle_string("A enemy projectile(id:"+id.out+") moves: ["+model.array_row[position.row]+","+(position.column+10).out+"] -> ["+model.array_row[position.row]+","+(position.column).out+"]%N")
          end

          if position.column+10<1 then
