note
	description: "Summary description for {PYLON_PROJECTILE}."
	author: ""
	date: "$Date$"
	revision: "$Revision$"
...
feature
     ...
     local
         ...
         do
          ...
          if position.column < 1 and( position.column +2)>=1 and interaction =false and status = true then
          ...
          elseif interaction = false and status = true then
              	model.fill_enemy_proj_toggle_string("A enemy projectile(id:"+id.out+") moves: ["+model.array_row[position.row]+","+(position.column+2).out+"] -> ["+model.array_row[position.row]+","+(position.column).out+"]%N")
          end

          if position.column+2<1 then
