...
feature
	make
	 ...
	 do
               ...
               across 1 |..| 5 is p loop
               	if position0.column+ p > model.board_column then
	 		       update_status0(false)
	 		    end
		              across model.friendly_projectile is f loop

		                                             if f.position0.row ~ position0.row and f.position0.column ~ (position0.column+p) and f.status0= true and not( f.id0 = id0) and status0 = true  then
		              ...
		              across model.enemy is e loop
		                                            if e.position.row ~ position0.row and e.position.column ~ (position0.column+p) and  not( e.id = id0) and status0 = true and e.alive = true  then
			                                                  ...
			                                                  print("friendly proj colliding enemy , new enemy health: "+e.health_n.out+"enemy stat : "+e.alive.out+" frpooj stat: "+status0.out+" point of collision "+(position0.column+p).out+"%N")
                                                    end
       ...
       position0.column:= position0.column+5
	 end
