note
	description: "Summary description for {FIGHTER_PROJECTILE_ES}."
	author: ""
	date: "$Date$"
	revision: "$Revision$"

class
	FIGHTER_PROJECTILE_ES
...
feature
     ...
     local
         ...
         do
          ...
          if position.column < 1 and( position.column +6)>=1 and interaction =false and status = true then
          ...
          elseif interaction = false and status = true then
              	model.fill_enemy_proj_toggle_string("A enemy projectile(id:"+id.out+") moves: ["+model.array_row[position.row]+","+(position.column+6).out+"] -> ["+model.array_row[position.row]+","+(position.column).out+"]%N")
          end

          if position.column+6<1 then
