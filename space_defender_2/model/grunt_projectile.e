note
	description: "Summary description for {GRUNT_PROJECTILE}."
	author: ""
	date: "$Date$"
	revision: "$Revision$"
       ...
       local
         ...
         do
          ...
          position.column:= position.column- 4
           if position.column < 1 and( position.column +4)>=1 and interaction =false and status = true then
          ...
          elseif interaction = false and status = true then
              	model.fill_enemy_proj_toggle_string("A enemy projectile(id:"+id.out+") moves: ["+model.array_row[position.row]+","+(position.column+4).out+"] -> ["+model.array_row[position.row]+","+(position.column).out+"]%N")
          end

          if position.column+4<1 then
