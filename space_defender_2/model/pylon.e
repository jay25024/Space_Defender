...
feature-- Actions
   ...
   do
   	...
   	across model.enemy is e loop
   	  if ((position.row-e.position.row).abs + (position.column-e.position.column).abs) > vision or e.alive = false or alive = false then else
   	     if   e.health_n + 10 > e.health_d   then  e.health_mutator(e.health_d)  else e.health_mutator(e.health_n +10)end
    ...
    do
 	...
 	fire
      ...
      do
       ...
       model.enemy_projectile.force((create {PYLON_PROJECTILE}.make),model.enemy_projectile.count+1)
